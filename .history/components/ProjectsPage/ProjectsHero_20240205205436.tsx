...
const ProjectsHero = () => {
  ...
  const filters = [
    ...
    },
    {
      ...
      link:  currentLocale==="en" ? `/${currentLocale}/projects?service=landscaping`: `/${currentLocale}/projects?service=aménagementpaysager`
    }
  ]
    ...
    <section className={`section__special ${styles.projects__section}`}>
      <div className={`container ${styles.projects__container}`}>
        ...
        <div className={styles.filters}>
          {
            filters.map((data, i) => (
              <Link href={data.link} className={styles.filter__container} key={i}>
                {data.name}
              </Link>
            ))
          }
