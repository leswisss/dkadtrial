...
const AboutMission = () => {
  ...
  const splitwords = (phrase: string) => {
    ...
    phrase.split(" ").forEach((word, i) => {
      body.push(
        <p key={`word-${i}`}>
          <span variants={valuesAnim} animate={activeSlide===i ? "enter" : "initial"}>{word}</motion.span>
        </p>
      );
    });
  ...
  const splitwordSpecial = (phrase: string, i: number) => {
    let body: JSX.Element[] = [];
    phrase.split(" ").forEach((word, i) => {
      body.push(
        <p key={`word-${i}`}>
          <motion.span variants={valuesAnim} animate={activeSlide===i ? "enter" : "initial"}>{word}</motion.span>
        </p>
      );
    });
              ...
              <div className={styles.v__content}>
                <div className={styles.v__header}>
                  {ValuesData.map((data, i) => (
                    <div className={styles.v__headerr} key={i}>{splitwords(data.name)}</div>
                  ))}
                </div>
