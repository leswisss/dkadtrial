...
const AboutMission = () => {
  ...
  const changeSlide = () => {
    setActiveSlide((prevSlide) => {
      const nextSlide = (prevSlide + 1) % Val.length;
      updateButtonWidths(nextSlide);
      return nextSlide;
    });
  };

  const updateButtonWidths = (nextSlide: number) => {
    setButtonWidths(() => (nextSlide === 0 ? new Array(Val.length).fill(0) : new Array(Val.length).fill(100)));
  };
