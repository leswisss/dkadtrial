...
const AboutMission = () => {
  ...
  const changeSlide = () => {
    setActiveSlide((prevSlide) => {
      const nextSlide:number = (prevSlide + 1) % ValuesData.length;
      setCompletedSlides((prevSlides) => [...prevSlides, prevSlide]);
      return nextSlide;
    });
  };
  ...
  useEffect(() => {
    slideInterval.current = setInterval(() => {
      setProgress((prevProgress) => {
        ...
        return prevProgress + 100 / 7; // Increase progress by approximately 14.28% every second
      });
  ...
  }, [activeSlide]); // Reset the timer and progress whenever activeSlide changes
                ...
                {ValuesData.map((data, i) => (
                  ...
                  >
                    <div
                      ...
                      style={{
                        width: `${activeSlide === index ? progress : completedSlides.includes(index) ? 100 : 0}%`,
                        transition: 'width 1s',
                      }}
                    ></div>
                  </div>
