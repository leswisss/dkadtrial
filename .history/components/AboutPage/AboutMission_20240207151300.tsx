...
const AboutMission = () => {
  ...
  useEffect(() => {
    const progressWrappers = document.querySelectorAll('.progress__wrapper');
    let gap = 0;

    if (progressWrappers.length > 1) {
      const firstOffset = progressWrappers[0].offsetLeft;
      const secondOffset = progressWrappers[1].offsetLeft;
      gap = secondOffset - firstOffset - progressWrappers[0].offsetWidth;
    }

    const progressElements = document.querySelectorAll('.progress');

    progressElements.forEach((progress) => {
      progress.style.transform = `translateX(${gap}px)`;
    });
  }, []);
