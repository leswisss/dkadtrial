...
const AboutMission = () => {
  ...
  useEffect(() => {
    if (activeSlide) {
      // Animation 1: Opacity from 1 to 0 with a transition duration of 0.5s
      controls.start({ opacity: 0, transition: { duration: 1 } })
        .then(() => {
          // Animation 2: Top from 0 to 100 immediately with no transition
          controls.start({ top: '100%', transition: { duration: 0 } });
        })
        .then(() => {
          // Animation 3: Opacity from 0 to 1 immediately with no transition
          controls.start({ opacity: 1, transition: { duration: 0 } });
        })
        .then(() => {
          // Animation 4: Top from 100% to 0 with a transition duration of 0.5s
          controls.start({ top: '0%', transition: { duration: 0.5 } });
        });
    }
  ...
  const splitwords = (phrase: string ) => {
    ...
    phrase.split(" ").forEach((word, i) => {
      body.push(
        <p key={`word-${i}`}>
          <motion.span initial={{ opacity: 1, top: '0%' }}
            ...
            <div className={styles.values__content}>
              ...
              <div className={styles.v__content}>
                <div className={styles.v__header}>
                  {splitwords(ValuesData[activeSlide].name)}
                </div>
                <div className={styles.p__text}>
                  {splitwords(ValuesData[activeSlide].text)}
                </div>
              </div>
