...
const Navbar = ({ lang }: { lang: string }) => {
  ...
  return (
    <>
      <header className={currentPathname==="/en" || currentPathname==="/fr" ? styles.navigation : `${styles.navigation} ${styles.navigation__black}`}>
        <div className={`container ${styles.navigation__container}`}>
          <div className={styles.nav__left}>
            ...
            <div className={styles.left__menu}>
              {NavMenu.map((data, i) => (
                <Magnetic key={i}>
                  <Link key={`link-${i}`} href={lang==="en" ? `/${lang}${data.link[0]}` : `/${lang}${data.link[1]}`}>
                    {navigation__menu[i]}
                  </Link>
                </Magnetic>
              ))}
