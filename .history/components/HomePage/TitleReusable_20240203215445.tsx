...
const TitleReusable = ({title, text, classNames}: {title: string, text: string, classNames?: string}) => {
  ...
  const splitwords = (phrase: string, condition: boolean) => {
    ...
    phrase.split(" ").forEach((word, i) => {
      body.push(<p key={`word-${i}`} className={styles.p}><span style={{top: condition ? "0%" : "-100%"}} >{word}</span></p>);
    });
  ...
  return (
    <div className={`section__header ${classNames}`}>
      <div className="header__overflow" ref={ref}>
        <h1 className="large__font" style={{textTransform: "capitalize"}}>{splitwords(title, inView)}</h1>
      </div>
      <p className="p__font">
        {text}
      </p>
