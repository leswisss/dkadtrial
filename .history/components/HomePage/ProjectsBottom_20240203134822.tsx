...
const ProjectsBottom = () => {
  ...
  const animateSlide = (slideNumber: number, reveal: boolean) => {
    ...
    gsap.to(background, { clipPath: clipPathValue, duration: 1, ease: 'power4.out' });
      ...
      <div className={styles.projects__bottom}>
        <div className={styles.projects__content} ref={container}>
          ...
          <div className={styles.project__images}>
            {ProjectsContent.map((content, i) => (
              <div className={`${styles.project__image} ${activeProject===0 ? styles.first__image : ""}`} key={i} ref={(el) => (imgRefs.current[i] = el)}>
                <Image
                  src={content.image2}
                  fill
                  alt="Project Image"
