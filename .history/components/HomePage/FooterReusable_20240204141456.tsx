...
const FooterReusable = ({ data }: { data: DataProps }) => {
  ...
  const splitwords2 = (phrase: string, condition: boolean) => {
    let body: JSX.Element[] = [];
    phrase.split(" ").forEach((word, i) => {
      body.push(<p key={`word-${i}`}><span style={{top: condition ? "0%" : "-100%"}} >{word}</span></p>);
    });
        ...
        <div className={styles.left}>
          <div className={styles.left__content}>
            <div className={styles.left__top}>
              <div className={styles.header__one} ref={ref}>{data.title}</div>
              <div className={styles.header__two} ref={ref2}>{data.title2}</div>
            </div>
