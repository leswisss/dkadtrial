...
const HomeContact = () => {
  ...
  const splitwords = (phrase: string, condition: boolean) => {
    ...
    phrase.split(" ").forEach((word, i) => {
      body.push(<p key={`word-${i}`} className={styles.overflow}><span className={word==="conception" || "design" ? styles.text__fade : "" } style={{top: condition ? "0%" : "-100%", transition: "top 0.7s linear"}} >{word}</span></p>);
    });
      ...
      <div className={`container ${styles.contact__container}`}>
        <div className={styles.contact__top}>
          <div className={styles.top__one} ref={ref}>
            <p className={styles.p__text}>{splitwords(text1, inView)}</p>
          </div>
          <div className={styles.line}></div>
          <div className={styles.top__two} ref={ref2}>
            <p className={`${styles.p__text} ${styles.align}`}>{splitwords(text2, inView2)}</p>
          </div>
        </div>
