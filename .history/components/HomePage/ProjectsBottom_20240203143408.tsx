...
const ProjectsBottom = () => {
  ...
  const animateSlide = (slideNumber: number, reveal: boolean) => {
    ...
    gsap.to(background, { clipPath: clipPathValue, duration: 0.7, ease: "power4.out" });
    gsap.to(img, { clipPath: clipPathValue, duration: 0.7, ease: "power4.out" });
  };
      ...
      <div className={styles.projects__bottom}>
        <div className={styles.projects__content} ref={container}>
          ...
          <div className={styles.overlay}>
            <div onClick={handleLeftClick} onMouseEnter={() => {
          setCursor({ active: true, i0 });
        }}
        onMouseLeave={() => {
          setCursor({ active: false, i0 });
        }}></div>
            <Link href="/" />
            <div onClick={handleRightClick}></div>
          </div>
          <Cursor active={activeProject} length={ProjectsContent.length} cursor={cursor} />
        </div>
      </div>
    </>
  );
