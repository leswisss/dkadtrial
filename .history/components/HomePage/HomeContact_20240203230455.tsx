...
const HomeContact = () => {
  ...
  const splitwords = (phrase: string, condition: boolean) => {
    ...
    phrase.split(" ").forEach((word, i) => {
      body.push(<p key={`word-${i}`}><span className={word==="conception" || "design" ? styles.text__fade : ""} style={{top: condition ? "0%" : "-100%"}} >{word}</span></p>);
    });
      ...
      <div className={`container ${styles.contact__container}`}>
        <div className={styles.contact__top}>
          ...
          <div className={styles.top__two} ref={ref2}>
            <p className={`${styles.p__text} ${styles.align}`}>{splitwords(data.title, inView)}</p>
          </div>
        </div>
