...
const HomeContact = () => {
  ...
  const splitwords = (phrase: string, condition: boolean) => {
    ...
    phrase.split(" ").forEach((word, i) => {
      body.push(<p key={`word-${i}`} className={styles.overflow}><span className={word==="conception" || "design" ? styles.text__fade : "" } style={{top: condition ? "0%" : "-100%", transition: "top"}} >{word}</span></p>);
    });
