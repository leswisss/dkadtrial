...
const ProjectsBottom = () => {
  ...
  const animateSlide = (slideNumber: number, reveal) => {
    const marquee = marqueeRefs.current[slideNumber];
        ...
        <div className={styles.projects__content} ref={container}>
          <motion.div style={{ y }} className={styles.background__images}>
            {ProjectsContent.map((content, i) => (
              <div className={`${styles.background__image} ${activeProject===0 ? styles.first__background : ""}`} key={i}>
                <Image
                  src={content.image}
                  fill
                  alt="Project Image"
