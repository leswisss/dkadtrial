  ...
  return (
    ...
    >
      <div className={styles.directives}>
        <Icon icon="la:angle-left" style={{opacity: cursor.index===0 && active > 0 ? 1 : 0.5}}/> <span>{active + 1} / {length}</span>{" "}
        <Icon icon="la:angle-right" style={{opacity: cursor.index===1 && active < length-1 ? 1 : 0.5}} />
      </div>
    </motion.div>
  );
};
